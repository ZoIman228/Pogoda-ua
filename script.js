...
function crossfadeTo(url) {
  ...
  requestAnimationFrame(() => {
    ...
    // Consolidated script.js - single theme block, clean structure
    const API_KEY = "f0d6a0da789f48808a6123315263005";
    ...
    // Weather UI elements (may be absent on login page)
    const input = document.getElementById("city-input");
    const searchBtn = document.getElementById("search-btn");
    const result = document.getElementById("weather-result");
    ...
    if (searchBtn && input) {
      searchBtn.addEventListener("click", () => {
        ...
        if (!q) {
          if (result) result.innerHTML = '<div class="error">Введіть назву міста</div>';
          return;
        }
    ...
    function showApp() {
      ...
      if (userJson) {
        try {
          ...
          if (heading) heading.textContent = `Прогноз погоди — ${user.name || user.email}`;
        } catch (e) {}
      }
    }
    ...
    if (storedUser) showApp(); else showAuth();

    if (regBtn) {
      regBtn.addEventListener("click", async () => {
        if (!regEmail.value || !regPassword.value) {
          regMsg.textContent = "Введіть email і пароль";
          regMsg.className = "error";
          return;
        }
        if (regPassword.value !== regPassword2.value) {
          regMsg.textContent = "Паролі не співпадають";
          regMsg.className = "error";
          return;
        }
        ...
        try {
          ...
          const data = await resp.json();
          if (!resp.ok) {
            regMsg.textContent = data && data.error ? data.error : "Помилка реєстрації";
            regMsg.className = "error";
          } else {
            regMsg.textContent = "Реєстрація успішна";
            regMsg.className = "success";
            const userObj = { id: data.id, email: data.email, name: regName.value.trim() || data.name };
            setUser(userObj);
            regEmail.value = regName.value = regPassword.value = regPassword2.value = "";
          }
        } catch (e) {
          regMsg.textContent = "Помилка мережі";
          regMsg.className = "error";
        } finally {
    ...
    if (loginBtn) {
      loginBtn.addEventListener("click", async () => {
        if (!loginEmail.value || !loginPassword.value) {
          loginMsg.textContent = "Введіть email і пароль";
        ...
        try {
          ...
          const data = await resp.json();
          if (!resp.ok) {
            loginMsg.textContent = data && data.error ? data.error : "Помилка входу";
            loginMsg.className = "error";
          } else {
          ...
          loginMsg.textContent = "Помилка мережі";
          loginMsg.className = "error";
        } finally {
